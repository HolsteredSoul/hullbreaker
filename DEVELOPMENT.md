...
`src/enemy-navigation.js` provides bounded velocity sampling and collision-aware spawn placement. Inputs use the previous frame distance and measured scroll speed, so braking holds an enemy relative to the installation rather than allowing scrolling cover to hit it. `tests/navigation.test.js` checks steering, safe spawns, 24 canyon/moon scenarios across three seeds, and armoured turret burst timing/cancellation. `src/help.js` builds the native help dialog from the shared pickup palette, with normal focus restoration and modal keyboard behavior.
